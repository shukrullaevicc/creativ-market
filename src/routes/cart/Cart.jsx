...
import { addToCart, removeFromCart, calculateTotals, deleteCart } from '../../redux/slice/cartSlice';
    ...
    <div className="flex flex-col items-end p-5 min-h-screen">
          ...
          {cart.map((product) => (
            <tr key={product.id} className="bg-white">
              <td className="p-4 flex items-center gap-2 justify-center border border-gray-300">
                <img src={product.images} alt="" width="50" height="50" className="object-cover" />
                {product.name}
              </td>
              <td className="p-4 text-center border border-gray-300">${product.price}</td>
              <td className="p-4 text-center border border-gray-300">
                <div className="flex items-center justify-center gap-2">
                  <button
                    onClick={() => dispatch(removeFromCart(product))}
                    className="flex items-center justify-center p-2 bg-gray-200 text-blue-400 rounded"
                  >
                    <AiOutlineMinus />
                  </button>
                  <p className="text-lg">{product.quantity}</p> {/* Mahsulotning miqdori */}
                  <button
              ...
              <td className="p-4 text-center border border-gray-300">
                <button className="p-2 bg-red-100 rounded-full">
                  <img onClick={() => dispatch(deleteCart(product))} src={x} alt="Remove" />
                </button>
              </td>
            </tr>
          ))}
      ...
      <div className="w-full max-w-xs bg-white p-5 rounded-lg shadow-lg mt-5 text-right">
        <div className="flex justify-between py-2 border-b border-gray-200">
          ...
          <span className="text-base text-blue-500">${total.toFixed(2)}</span> {/* Subtotal */}
        </div>
        <div className="flex justify-between py-2 border-b border-gray-200">
          <span className="text-base text-gray-600">Shipping Fee</span>
          <span className="text-base text-blue-500">${shippingFee}</span> {/* Shipping Fee */}
        </div>
        <div className="flex justify-between py-2 font-bold text-lg">
          <span className="text-base text-gray-600">Total</span>
          <span className="text-blue-500">${(total + shippingFee).toFixed(2)}</span> {/* Total */}
        </div>
      </div>
    </div>
  );
